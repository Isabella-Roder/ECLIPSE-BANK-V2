...
- Analytics em Python: módulo `analytics/`, isolado do backend Java, roda sob demanda (não é um serviço que fica no ar). Fluxo: (1) com o backend **parado** (H2 file mode só aceita 1 conexão), roda `java -cp <h2.jar> org.h2.tools.Shell` com o script `analytics/export.sql`, que usa `CSVWRITE` pra exportar `MOVIMENTACOES` + `CONTAS` + `USUARIOS` (só as colunas necessárias, sem `SENHA_HASH`) pra `analytics/dados/movimentacoes.csv`; (2) `analytics/relatorios/relatorio.py` (ambiente virtual próprio em `analytics/.venv`, com `pandas` e `matplotlib`) lê o CSV, separa movimentações em receita/despesa por `TIPO`, calcula totais por usuário, saldo médio e maiores movimentações, gera um gráfico de gastos por tipo (`grafico_despesas.png`) e monta um `relatorio.html` com tudo. CSVs, `.venv` e relatórios gerados (HTML/PNG) estão no `.gitignore` — contêm dados reais de usuários e nunca devem ir pro Git; só o código (`export.sql`, `relatorio.py`) é versionado.

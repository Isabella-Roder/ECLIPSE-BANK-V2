...
- Detecção de movimentação suspeita: no helper privado `MovimentacaoService.registrar(...)` (usado por `depositar`/`sacar`/`transferir`/`fazerPix`), compara o valor da movimentação com a média das movimentações anteriores de mesmo `TipoMovimentacao` naquela conta (`MovimentacaoRepository.calcularValorMedioPorContaETipo`); só avalia a partir da 3ª movimentação do mesmo tipo (`countByContaIdAndTipo`), e se o valor novo for mais de 5x a média, registra `AcaoAuditoria.MOVIMENTACAO_SUSPEITA` via `AuditoriaService`. É só um alerta na auditoria — nunca bloqueia a operação.
- Analytics em Python: módulo `analytics/`, isolado do backend Java. Duas formas de uso: (1) script manual `analytics/relatorios/relatorio.py` — roda sob demanda, salva `relatorio.html` + `grafico_despesas.png` em arquivo; (2) API `analytics/api.py` (FastAPI + uvicorn, mesmo `.venv`), endpoint `GET /relatorio` gera o relatório na hora e devolve como HTML. Os dois reaproveitam `analytics/export.sql`, que usa `CSVWRITE` do H2 pra exportar `MOVIMENTACOES` + `CONTAS` + `USUARIOS` (só as colunas necessárias, sem `SENHA_HASH`) pra `analytics/dados/movimentacoes.csv`; a lógica em pandas separa movimentações em receita/despesa por `TIPO`, calcula totais por usuário, saldo médio e maiores movimentações. **Importante**: `spring.datasource.url` no `application.properties` tem `AUTO_SERVER=TRUE` (sem `DB_CLOSE_ON_EXIT=FALSE`, que é incompatível com `AUTO_SERVER`) — isso permite que o Python se conecte ao mesmo arquivo H2 enquanto o backend Java está rodando, sem precisar pará-lo. A URL do Python (`api.py`) deve usar o mesmo formato `jdbc:h2:file:<caminho absoluto>;AUTO_SERVER=TRUE` (não `tcp://`, cuja porta muda a cada start) — o H2 detecta o lock file e redireciona sozinho pra conexão TCP certa. CSVs, `.venv` e relatórios gerados (HTML/PNG) estão no `.gitignore` — contêm dados reais de usuários e nunca devem ir pro Git; só o código (`export.sql`, `relatorio.py`, `api.py`) é versionado.
